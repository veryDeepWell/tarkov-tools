(function () {
  ...
  function paintStatusUI() {
    ...
    if (el) el.textContent = run.on ? ("Фон ВКЛ · каждые " + mins + " мин · последний " + last + (meta.count != null ? " (" + meta.count + ")" : "")) : ("Фон выкл · последний " + last);
    var remainMs = run.on && run.nextSnapAt ? Number(run.nextSnapAt) - Date.now() : null;
    if (cd) {
      if (run.on) { cd.textContent = flying ? "След.: идёт снимок…" : ("След.: " + fmtRemain(remainMs)); cd.className = "countdown on"; }
      else { cd.textContent = "Отсчёт выкл"; cd.className = "countdown"; }
    }
    try {
      if (window.TarkovPoll && TarkovPoll.reportMini) TarkovPoll.reportMini("tarkovtool-price-track.html", !!run.on, run.on ? ("Next: " + fmtRemain(remainMs)) : "idle");
    } catch (e) {}
  }
  ...
  function stopBg() { window.__ttStartLock = false; clearPollTimer(); paintStatusUI(); }
