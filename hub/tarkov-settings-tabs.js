...
(function () {
  ...
  function ensureCss() {
    ...
    s.textContent =
      ...
      ".tt-tool-sound-list{max-height:200px;overflow:auto;margin:0 0 8px;}" +
      ".tt-tool-sound-row{display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--border);}" +
      ".tt-tool-sound-row:last-child{border-bottom:0;}" +
      ".tt-tool-sound-row label{flex:1;min-width:0;}" +
  ...
  TarkovTools.openSettings = function () {
    ...
    if (!bg) {
      ...
      bg.innerHTML =
        ...
        '<div class="field"><label>' + t("common.toolTips", "Подсказки инструментов") + '</label>' +
        ...
        '</button>' +
        '<p class="meta" style="margin:14px 0 8px">' + t("common.soundKinds", "Типы звуков") + "</p>" +
        '<div id="tt-set-kinds">' +
        kindRow("ok", t("common.uiSound", "Интерфейс / трекер")) +
        ...
        '<div class="tt-tool-sound-list" id="tt-set-tool-sounds"></div></div>' +
        ...
        '<p class="meta" style="margin:0 0 10px">' + t("common.hiddenHint", "Скрытые инструменты не показываются в хабе") + '</p>' +
    ...
    /* Per-tool mute: primarily live tools (they notify); fall back to all catalog if registry missing */
    var toolSoundBox = document.getElementById("tt-set-tool-sounds");
    if (toolSoundBox) {
      var liveList = [];
      try {
        if (window.TarkovToolKind && TarkovToolKind.liveEntries) {
          liveList = TarkovToolKind.liveEntries().map(function (e) { return e.file; });
        }
      } catch (eL) {}
      if (!liveList.length) {
        liveList = catalog.filter(function (t) { return t && t.kind === "live"; }).map(function (t) {
          return (t.file || "").split("/").pop();
        });
      }
      if (!liveList.length) {
        liveList = ["tarkovtool-price-track.html", "tarkovtool-price-alarm.html", "tarkovtool-restock.html"];
      }
      toolSoundBox.innerHTML = liveList.map(function (file) {
        var base = String(file || "").split("/").pop();
        var title = base;
        try {
          if (window.TarkovI18n && TarkovI18n.toolTitle) {
            var ti2 = TarkovI18n.toolTitle(base);
            if (ti2 && ti2.indexOf("tool.") !== 0) title = ti2;
          }
        } catch (eT) {}
        if (title === base) {
          for (var ci = 0; ci < catalog.length; ci++) {
            if (catalog[ci] && (catalog[ci].file || "").indexOf(base) >= 0) {
              title = catalog[ci].title || base;
              break;
            }
          }
        }
        var on = get("tarkovSoundTool." + base, "1") !== "0";
        return '<div class="tt-tool-sound-row"><input type="checkbox" class="tt-tool-snd" data-file="' +
          escAttr(base) + '"' + (on ? " checked" : "") + '> <label>' + escHtml(title) + "</label></div>";
      }).join("") || ('<p class="meta">' + t("common.noTools", "Нет инструментов") + "</p>");
    }
