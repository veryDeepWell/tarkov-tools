...
(function () {
  ...
  function wireCards() {
    ...
    document.querySelectorAll(".tool-help[data-help]").forEach(function (btn) {
      btn.onclick = function (e) {
        e.preventDefault();
        e.stopPropagation();
        var f = btn.getAttribute("data-help");
        var tip = document.getElementById("toolHelpPop");
        if (!tip) {
          tip = document.createElement("div");
          tip.id = "toolHelpPop";
          tip.className = "tool-help-pop";
          document.body.appendChild(tip);
        }
        var title = f, desc = "";
        try {
          if (window.TarkovI18n) {
            var ti = TarkovI18n.toolTitle(f);
            var de = TarkovI18n.toolDesc ? TarkovI18n.toolDesc(f) : "";
            if (ti && ti.indexOf("tool.") !== 0) title = ti;
            if (de && de.indexOf("tool.") !== 0) desc = de;
          }
        } catch (err) {}
        if (!desc) {
          try {
            var cat = (window.TarkovHubCATALOG || []).find(function (x) { return x.file === f; });
            if (cat) { title = cat.title || title; desc = cat.description || ""; }
          } catch (err2) {}
        }
        tip.innerHTML = "<b>" + String(title).replace(/</g,"<") + "</b><div style=\"margin-top:6px\">" +
          String(desc || "").replace(/</g,"<") + "</div>";
        ...
        tip._hide = setTimeout(function () { tip.style.display = "none"; }, 4000);
      };
    });
    document.querySelectorAll(".tool[data-open]").forEach(function (el) {
      el.onclick = function (e) {
        if (e.target.closest && (e.target.closest(".tool-pin") || e.target.closest(".tool-help"))) return;
