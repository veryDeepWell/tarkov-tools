 ...
 * Live tools must stay in-layout (opacity/visibility only) — left:-9999 / 1x1 freezes timers in Chrome. */
function layoutFramePool() {
  ...
  if (expanded && host) {
    ...
    pool.style.cssText =
      "position:fixed;left:-4000px;top:0;width:1100px;height:800px;overflow:hidden;z-index:0;margin:0;padding:0;border:0;";
    Object.keys(frames).forEach(function (f) {
      var ifr = frames[f];
      if (!ifr) return;
      ifr.style.cssText =
        "position:absolute;left:0;top:0;width:1100px;height:800px;border:0;opacity:0;visibility:hidden;pointer-events:none;display:block;background:#0f1115;";
    });
  }
}
...
setInterval(function () {
  ...
  for (var i = 0; i < keys.length; i++) {
    try {
      ...
      if (ping) ifr.contentWindow.postMessage({ type: "tt-ping-status" }, location.origin);
    } catch (e) {}
  }
}, 1000);
