...
const frames = {};
const statusMap = {};
/** Always basename so status/notify match mini tabs with tools/ prefix */
...
function unreadCount(file) {
  try { return TarkovState.unreadForTool(toolKey(file)).length; } catch (e) { return 0; }
}
function unreadItems(file) {
  try { return TarkovState.unreadForTool(toolKey(file)).slice(0, 8); } catch (e) { return []; }
}
...
function ensureFrame(file) {
  ...
  frames[file] = ifr;
  statusMap[toolKey(file)] = statusMap[toolKey(file)] || { ready: false, running: false, label: "", ts: 0 };
  ifr.addEventListener("load", function () {
    statusMap[toolKey(file)] = Object.assign({}, statusMap[toolKey(file)] || {}, { ready: true, ts: Date.now() });
...
function closeTab(file) {
  ...
  destroyFrame(file);
  try { TarkovState.removeMiniTab(file); TarkovState.markToolRead(toolKey(file)); } catch (e) {}
  renderMiniList();
}
