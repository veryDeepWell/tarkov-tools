/*! hub-app recovery loader (sync XHR) — full app from last good commit + rename */
(function () {
  var urls = [
  ...
  function patch(code) {
    return code
      ...
      .split('postMessage({ type: "tt-ping-status" }, "*")').join(
        'postMessage({ type: "tt-ping-status" }, location.origin)'
      )
      .split('addEventListener("message", function (ev) {\n  const d = ev.data;\n  if (!d || typeof d !== "object") return;').join(
